...
class Generator(DEVSAtomicModel):
    ...
    def __init__(self, strID, globalVar, EDService, EDServiceRate, genEndTime, psgrPercent):
        ...
        # 오름차순으로 정렬된 timeTable 중 첫번째 요소를 도착시간으로 설정,여기서 arrival time은 승객 앞으로 차량이 가기까지의 시간인가?
        # 내가 알기로 이 값은 승객이 도착하게 되는 시간이네, 그러면 이 값은 승객이 있는 경우 1 없는 경우 0 으로 처리해서 boolean으로 처리해야할거 같은데
        # 가장 먼저 호출한 승객와 두번째로 호출한 승객 간의 시간 차이   
        self.arrivalTime = 0
    ...
    def funcOutput(self):
        if self.state == "GEN":
            ...
            self.arrivalTime=100 # 승객과 승객간의 호출 시간 간격을 100초로 고정  
            ...
            arr_node = arr_node[0] #arr_node는 리스트 형식으로 노드와 가장 가까운 애들부터 정렬되어있다. 이 중 가장 가까운 노드가 [0] 에 있다다
    ...
    #상태가 GEM이라면 현재 timeTable(언제 새로운 승객이 등장할지"를 시뮬레이션 시작 전에 미리 전부 계산해 놓은 리스트,오름차순)
    #내가 원하는 방향으로 가려면 이 로직이 아예 제거되야 하네 
